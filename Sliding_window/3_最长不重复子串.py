...
0704:while小循环的的条件怎么选，通常选择可以移动start，即可以造成窗口收缩的条件，因为我们需要在小循环中进行窗口收缩，大循环可以自动扩张窗口，无须考虑
0712:规范化滑动窗口模板，更新输出最好放到缩小窗口的循环外面，这样可以避免一些特殊情况造成的偏差
0716:注意将max_len = float('-inf')运行后，需要在最后判断一下是否没有进入循环
0718:这道题为什么可以自动更新end，原因在于start向前进的时候，不需要将end往回缩去比较，因为这样一定没有不回缩的长度长，这道题在找最大长度，因此无须回缩
     因此当前的自动更新end可以满足要求，但是这一点并不是一定可行，需要由题意决定，存在回缩的情况一般需要用到前缀和，也有他的固定特征，可参看1248
'''
    ...
    def lengthOfLongestSubstring(self, s: str) -> int:
        ...
        for end in range(len(s)):
            while s[end] in mark:
                ...
                start += 1
            mark.add(s[end])
            #更新输出最好放到缩小窗口的循环外面，这样可以避免一些特殊情况造成的偏差
